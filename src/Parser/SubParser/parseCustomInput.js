...
function validateCustomInputWithRegex(content, escapedDelimiter) {
  // 1. ^[0-9] 정수부를 잡는다
...
function validateDuplicateDelimiter(parts) {
...
function sumAllNumbers(parts) {
  return parts.map(n => parseFloat(n)).reduce((acc, curr) => acc + curr, 0);
}
export default function parseCustomInput(input) {
  ...
  validateCustomInputWithRegex(content, escapedDelimiter);

  // delimiter로 컨텐츠를 나눔.
  const parts = content.split(delimiter);
  validateDuplicateDelimiter(parts);
