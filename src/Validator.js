...
export class Validator {
    ...
    static validateInput = input => {
        ...
        if (!input) throw new Error(ERROR_MESSAGES.UNDEFINED_EXPRESSION);
        return input;
    }

    // 올바르게 선언된 구분자 예외 처리
    static validateCorrectlyDeclaredDelimiters = userInput => {
        if ('0' <= userInput[2] && '9' >= userInput[2]) throw new Error(ERROR_MESSAGES.CUSTOM_DELIMITER_CANNOT_BE_NUMBER);
    }
    ...
    static validateIncorrectlyDeclaredDelimiters = userInput => {
        if (userInput.includes(DELIMITER_CONSTANTS.CUSTOM_DELIMITER_START) && userInput.includes(DELIMITER_CONSTANTS.CUSTOM_DELIMITER_END)) {
            ...
            if (startDelimiter === 0) throw new Error(ERROR_MESSAGES.CUSTOM_DELIMITER_MUST_BE_ONE_CHARACTER);
            if (endDelimiter - startDelimiter !== 3) throw new Error(ERROR_MESSAGES.CUSTOM_DELIMITER_MUST_BE_DEFINED_AT_THE_BEGINNING_AND_ONE_CHARACTER);
    ...
    static validateNumbers = splitedString => {
        const numbers = splitedString.map((value) => {
            ...
            if (isNaN(num)) throw new Error(ERROR_MESSAGES.NON_NUMBERIC_VALUE);
        ...
        if (negativeNumbers.length > 0) throw new Error(ERROR_MESSAGES.NEGATIVE_NUMBER_NOT_ALLOWED);
    }
}