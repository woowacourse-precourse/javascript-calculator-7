...
- [ ]  문자열 덧셈 계산기 저장소를 fork하고 clone하는 것으로 시작한다.
- [ ]  기능을 구현하기 전 [README.md](http://README.md) 에 구현할 기능 목록을 정리해 추가한다.
...
- [ ]  ‘덧셈할 문자열을 입력해 주세요.’라는 문구를 사용자에게 보여준다.
    - [ ]  변수(userInput)를 설정하여, 위 문구를 사용자가 본 뒤 입력한 값을 받는다.
    - [ ]  Console.readLineAsync()으로 사용자 입력을 받는다.

### 2. 계산 1) 커스텀 문자를 사용하지 않은 경우

- [ ]  커스텀 문자가 아닌 경우를 알 수 있는 조건문을 만든다.
    - [ ]  userInput[짝수] === 숫자
    - [ ]  userInput[홀수] === 쉼표(,) || 콜론(:)
- [ ]  구분자(쉼표(,) or 콜론(:)을 기준으로 숫자를 분리하여 숫자만 추출한다.
- [ ]  더한 값을 받는 변수를 만든다.
- [ ]  해당 변수에 각 숫자의 합을 더한다.

### 2. 계산 2) 커스텀 문자를 사용한 경우

- [ ]  커스텀 문자가 맞는 경우를 수 있는 조건문을 만든다.
    - [ ]  1.커스텀 문자 구분자를 만드는 문자를 인지하는 조건문
    userInput[0] === ‘/’ && 
    userInput[1] === ‘/’ && 
    userInput[3] === ‘\’ && 
    userInput[4] === ‘n’
    - [ ]  2.커스텀 문자만 구분자로 사용되어있는지 인지하는 조건문
    userInput[2] === 함수의 각 숫자 사이에 기입 된 것 (userInput[6~ 끝까지])
- [ ]  userInput 의 "//"와 "\n" 사이에 위치하는 문자를 커스텀 구분자로 인식한다.
- 예를 들어 "//;\n1;2;3"과 같이 값을 입력할 경우 커스텀 구분자는 세미콜론(;)이며, 결과 값은
- [ ]  사용자 입력 값에서 숫자만 추출 한다.
- [ ]  더한 값을 받는 변수를 만든다.
- [ ]  해당 변수에 각 숫자의 합을 더한다.
...
- [ ]  (1) 커스텀 사용 하지 않은 경우와 (2) 커스텀 사용하는 경우를 제외하면 에러가 뜨는 조건을 만든다.
    - [ ]  에러시 "[ERROR] 잘못된 입력입니다." 문구를 나타나게 한다.
    - [ ]  에러를 발생시킨다.
    - [ ]  이후, 애플리케이션은 종료시킨다.
