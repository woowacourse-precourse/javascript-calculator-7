# javascript-calculator-precourse by @blaireinse
...
- 입력
  ...
  - [x] 문자열 양끝에 공백이 포함된 경우 이를 제거하고 계산 (에러 발생 X)
  - [x] 문자열 사이에 공백이 포함된 경우 에러 발생
- 기본 구분자를 사용하는 경우
  - [x] 입력된 문자열에 기본 구분자가 포함되어 있지 않은 경우 입력값 반환 (에러 발생 X)
- 커스텀 구분자를 사용하는 경우
  - [x] 커스텀 구분자가 명시되지 않은 경우 에러 발생
  - [x] 커스텀 구분자로 숫자가 입력된 경우 에러 발생
  - [x] 커스텀 구분자로 `\` 또는 `/`가 사용된 경우 허용
  - [x] 파싱 후 문자열에 커스텀 구분자가 포함되어 있지 않은 경우 입력값 반환 (에러 발생 X)
  - [x] 파싱 후 문자열에 문자가 없거나 공백으로만 이루어져 있을 경우 0 반환
- 구분자 파싱 후
  - [x] 빈 문자열일 경우 0 반환
  - [x] 숫자가 아닌 값이 들어 있을 경우 에러 발생
  - [x] 음수가 포함되어 있을 경우 에러 발생
