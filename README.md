...
- 단위 테스트(Unit Testing)를 통해 예외 상황과 함수 동작을 검증
- 테스트는 `Jest`를 사용했으며, 다음과 같은 **주요 함수**를 테스트했음
  - `getCustomSeperator`함수: 입력된 문자열이 커스텀 구분자를 올바르게 추출하는지 테스트
  - `validateUserInput`함수: 음수나 잘못된 형식의 입력을 **예외 처리**하며, 유효한 입력만을 허용하는지 검증
  - `computeResult`함수: 기본 구분자 및 커스텀 구분자로 나눠진 숫자의 합계를 정확히 계산하고, NaN값에 에러를 던지는지 검증
